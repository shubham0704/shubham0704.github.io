(() => {
  ...
  const variableGlossaries = {
    ...
    cphast: {
      ...
      "grid-loss": ["\\mathcal L_{\\mathrm{grid}}", "The full-state microgrid training objective: one-step mean-squared error over the composed DGU and line state."],
      "grid-samples": ["|\\mathcal I|", "The number of valid microgrid trajectory and time-index training pairs."],
      "grid-state": ["\\hat{x}_{t+1}", "The predicted composed microgrid state: DGU capacitor charge and converter flux together with transmission-line flux."],
      "dgu-charge": ["q_i=C_iV_i", "DGU capacitor charge. Example: bus voltage V_i multiplied by local capacitance C_i."],
      "dgu-flux": ["p_i=L_{t,i}I_{t,i}", "DGU converter-inductor flux linkage. Example: converter current I_{t,i} multiplied by inductance L_{t,i}."],
      "grid-incidence": ["B_{\\mathrm{inc}}", "The supplied oriented line-to-bus incidence matrix. Reconfiguration replaces it with the declared target feeder graph while reusing the learned local blocks."],
